...
2. API-first contracts: public behavior is owned by include/smb public headers.
...
The SMB architecture is organized into the following layers.

1. Core layer
- Files: include/smb/core, src/smb/core
- Responsibilities: bus lifecycle, routing, service management, message framing, compatibility checks.

2. Node layer
- Files: include/smb/node, src/smb/node
...
1. Public architecture interfaces must live in include/smb.
...
4. New reusable architecture building blocks should be promoted to include/smb when cross-node reuse is expected.
...
Architecture design decisions should map to requirement IDs in docs/REQUIREMENTS_CATALOG.md.
