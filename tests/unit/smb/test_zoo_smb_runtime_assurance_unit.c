...
void test_runtime_startup_with_compatible_protocol_succeeds(void)
{
    ...
    options.domain_profile_override_enabled = ZOO_TRUE;
    options.domain_profile_override = ZOO_DOMAIN_PROFILE_INDUSTRIAL;
...
void test_runtime_heartbeat_admission_rejects_empty_peer_id(void)
{
    if (!zoo_smb_is_ready())
    {
        TEST_IGNORE_MESSAGE("SMB runtime is not ready in this environment");
    ...
    options.heartbeat_interval_ms = 100U;
    options.failover_timeout_ms = 500U;
    options.local_wire_major = 1U;
    options.local_wire_minor = 3U;
    options.peer_wire_major = 1U;
    options.peer_wire_minor = 2U;
    ...
    TEST_ASSERT_EQUAL_HEX32((uint32_t)ZOO_SMB_OK, (uint32_t)zoo_smb_runtime_start(g_runtime));

    TEST_ASSERT_EQUAL_HEX32(
        (uint32_t)ZOO_SMB_ERROR_INVALID_STATE,
        (uint32_t)zoo_smb_runtime_report_peer_heartbeat(g_runtime, "", 2U));
}
...
int main(void)
{
    ...
    RUN_TEST(test_runtime_heartbeat_admission_rejects_empty_peer_id);
