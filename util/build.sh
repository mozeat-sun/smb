#!/bin/bash
################################################################################
# ZOO Utility Module Build Script
# Supports Unity testing framework
################################################################################

set -e

BUILD_DIR="/home/sky/zoo/util/build"
SOURCE_DIR="/home/sky/zoo/util"
...
echo -e "${GREEN}Building ZOO Utility Module${NC}"
...
if [ "$INSTALL_LIBS" = true ]; then
    ...
    echo -e "${BLUE}Use pkg-config --cflags --libs zoo-util to get compiler flags${NC}"
fi
