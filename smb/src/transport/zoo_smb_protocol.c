...
{
    ZOO_LOG_DEBUG("Starting message deserialization: buffer_size=%zu", buffer_size);
    ...
    if (result == ZOO_SMB_OK)
    {
        ZOO_LOG_DEBUG("Message deserialized successfully: msg_type=%u, payload_size=%u, result=%d",
                          msg_out->header.msg_type,
                          msg_out->header.payload_size);
    }
    else
    {
        ZOO_LOG_ERROR("Message deserialization failed: result=%d", result);
    }
